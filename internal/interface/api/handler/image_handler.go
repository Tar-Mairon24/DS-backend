...
func (h *ImageHandler) SaveImage(c *gin.Context) {
    ...
    if err != nil {
		logrus.Errorf("Failed to retrieve image: %v", err)
	...
	diskPath, urlPath, err := h.imageUseCase.GeneratePath(file.Filename, propertyID)
    if err != nil {
    ...
    savedImage, err := h.imageUseCase.SaveImage(c.Request.Context(), image)
    if err != nil {
        os.Remove(diskPath)
...
func (h *ImageHandler) GetImageByID(c *gin.Context) {
	...
	if err != nil {
		logrus.Errorf("Invalid image ID: %v", err)
	...
	if image == nil {
		logrus.Errorf("Image not found: %v", err)
...
func (h *ImageHandler) GetImagesByPropertyID(c *gin.Context) {
	...
	if err != nil {
		logrus.Errorf("Failed to get images: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get images"})
		return
	}
	if len(images) == 0 {
		logrus.Errorf("No images found for property ID: %d", propertyID)
...
func (h *ImageHandler) GetMainImageByPropertyID(c *gin.Context) {
	...
	if err != nil {
		logrus.Errorf("Failed to get main image: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get main image"})
		return
	}
	if image == nil {
		logrus.Errorf("No main image found for property ID: %d", propertyID)
...
func (h *ImageHandler) UpdateImage(c *gin.Context) {
	...
	if existing == nil {
		logrus.Errorf("Image not found: %d", id)
		c.JSON(http.StatusNotFound, gin.H{"error": "Image not found"})
		return
	}
