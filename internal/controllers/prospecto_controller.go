...
func (ctrl *ProspectoController) DeleteProspecto(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete prospecto", "details": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Prospecto deleted successfully"})
}
